...
class PriorityQueue:
    ...
    def enqueue(self, state, priority):
...
class Node:
    ...
    def __init__(self, state, parent=None):
        self.state = state
        self.parent = parent
        self.h = self.heuristic(state)   # heuristic value

    # used by heapq to compare nodes
    def __lt__(self, other):
        return self.h < other.h

    def heuristic(self, state):     # Manhattan Distance heuristic
        distance = 0
        for i in range(3):
            for j in range(3):
                value = state[i][j]
        ...
        while not pq.is_empty():
            ...
            for child_state in self.find_children(current.state):
                if tuple(map(tuple, child_state)) not in explored:
                    child_node = Node(child_state, current)
                    pq.enqueue(child_node, child_node.h)
