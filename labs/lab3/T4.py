...
# heapq: A Python module that provides an implementation of the min-heap data structure.
# Min-heap: A binary tree where the smallest element is always at the root.
# Used here to implement the priority queue for Best First Search.
...
class Node:
    ...
    def __init__(self, state, parent=None, g=0):
        ...
        self.h = self.heuristic(state)           # estimated cost to goal (Misplaced Tiles)
        self.f = self.g + self.h                 # total cost

    def __lt__(self, other):                     # needed for heapq
        return self.f < other.f

    def heuristic(self, state):                  # Misplaced Tiles heuristic
        misplaced = 0
        for i in range(3):
            for j in range(3):
